    ...
    if (center) {
      ...
      const loadedAddresses = center.addresses?.length
        ? center.addresses.map((a, i) => {
            const lines = a.lines && a.lines.length > 0 ? [...a.lines] : (a.address || "").split("\n");
            if (lines.length === 1) {
              lines.push("");
            }
            return {
              ...a,
              lines,
              state: a.state || DEFAULT_INDIAN_STATE,
              pincode: a.pincode || "",
              isDefault: a.isDefault ?? i === 0,
            };
          })
        : [{ id: `addr-1`, city: "Indore", state: DEFAULT_INDIAN_STATE, pincode: "", address: "", lines: ["", ""], isDefault: true }];
